<template>
  <div id="Page-main">
    ...
    >
      ...
      <div
        v-if="dialogVisible"
        style="width: 50%; padding: 10px; margin: 10px; border: 1px solid #999"
      >
        <div>{{ title }}</div>
        <div slot="footer">
          <div @click="dialogVisible = false">取 消</div>
          <div type="primary" @click="dialogVisible = false">确 定</div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    ...
    this.network.on("click", (params) => {
      console.log("点击", params, params.nodes[0]);
      let _id = params.nodes[0];
      let _view = this.nodesArray.filter((obj) => {
        if (obj.id == _id) return obj;
      });
      console.log("_view：", _view);
      this.title = _view[0]["label"];
      this.dialogVisible = true;
      // this.network.addEdgeMode();
    });
    this.network.on("oncontext", (params) => {
      console.log("右击", params.nodes);
    });
  },
  methods: {
    init() {
